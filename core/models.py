...
class LivySession(models.Model):
    ...
    def refresh_from_livy(self, save=True):
        ...
        if livy_response != False:
            ...
            if livy_response.status_code == 404:
                ...
                try:
                    spark_app_id = SparkAppAPIClient.get_application_id(
                ...
                logger.debug('error: livy request http code: %s' %
                             livy_response.status_code)
                logger.debug('error: livy request content: %s' %
                             livy_response.content)
                return False
    ...
    def start_session(self):
        ...
        for x in range(0, 100):
            try:
                ...
                logger.debug('port %s open, using for LivySession' %
                             spark_ui_port)
    ...
    def ensure_active_session_id(session_id):
        ...
        if session_id == None:
            ...
            results = polling.poll(
                lambda: new_ls.refresh_from_livy() == 'idle', step=5, timeout=120)
        ...
        # if passed session id matches and status is idle or busy
            ...
            if session_id == active_ls.session_id:
                ...
                if active_ls.status in ['idle', 'busy']:
                    logger.debug(
                        'active livy session found, state is %s, ready to receieve new jobs, submitting livy job' % active_ls.status)

                else:
                    logger.debug(
                        'active livy session is found, state %s, but stale, restarting livy session' % active_ls.status)
                ...
                logger.debug(
                    'requested livy session id does not match active livy session id')
                return None
    ...
    def restart_session(self):
        ...
        logger.debug('polling for Livy session to start...')
        results = polling.poll(lambda: new_ls.refresh_from_livy(
        ), check_success=livy_session_ready, step=5, timeout=120)
...
class Job(models.Model):
    ...
    publish_set_id = models.CharField(
        max_length=255, null=True, default=None, blank=True)
    ...
    def refresh_from_livy(self, save=True):
        ...
        if livy_response.status_code in [400, 404]:
            ...
            logger.debug(
                'error retrieving information about Livy job/statement')
    ...
    def get_spark_jobs(self):
        ...
        if ls and type(ls) == LivySession:
            ...
            try:
                filtered_jobs = SparkAppAPIClient.get_spark_jobs_by_jobGroup(
                    ls, ls.appId, self.id)
            except:
    ...
    def get_detailed_job_record_count(self, force_recount=False):
        ...
        if 'detailed_record_count' in self.job_details_dict.keys() and not force_recount:
            ...
            if r_count_dict['records'] != 0:
                r_count_dict['success_percentage'] = round(
                    (float(r_count_dict['records']) / float(r_count_dict['records'])), 4)
            else:
    ...
    def _get_parent_jobs(self, job, ld):
        ...
        # get parent job(s)
        # reverse many to one through JobInput model
        parent_job_links = job.jobinput_set.all()
                ...
                if edge_id not in [edge['id'] for edge in ld['edges']]:
                    ...
                    if 'input_filters' in self.job_details_dict:
                        ...
                        if 'job_specific' in self.job_details_dict['input_filters'].keys() and str(from_node) in self.job_details_dict['input_filters']['job_specific'].keys():
                            ...
                            # get job_spec_dict
                            job_spec_dict = self.job_details_dict['input_filters']['job_specific'][str(
                                from_node)]
                        ...
                        logger.debug(
                            'no input filters were found for job: %s' % self.id)
                        edge_dict = {
                            'id': edge_id,
                            'from': from_node,
                            'to': to_node,
    ...
    def validation_results(self, force_recount=False):
        ...
        # validation tests run, loop through
            ...
            results['failure_count'] = Record.objects(
                job_id=self.id, valid=False).count()
            ...
            for jv in self.jobvalidation_set.all():
                results['validation_scenarios'].append(
                    jv.validation_scenario_id)

            # save to job_details
            logger.debug(
                "saving validation results for %s to job_details" % self)
            ...
            if 'dbdm' in self.job_details_dict.keys() and 'dbdd' in self.job_details_dict['dbdm'].keys() and self.job_details_dict['dbdm']['dbdd'] != None:
                ...
                try:
                    ...
                    if dbdm['matches'] == None and dbdm['misses'] == None:
                        ...
                        # misses
                        dbdm['misses'] = self.get_records().filter(
                            dbdm=False).count()
                    ...
                    logger.debug(
                        'Job claims to have dbdd, but encountered error while retrieving: %s' % str(e))
                    return {}

            else:
                logger.debug(
                    'DPLA Bulk comparison not run, or no matches found.')
                return False
    ...
    def publish(self, publish_set_id=None):
        ...
        # debug
        logger.debug('publishing job #%s, with publish_set_id %s' %
                     (self.id, publish_set_id))
    ...
    def remove_mapping_failures_from_db(self):
        ...
        mc_handle.combine.index_mapping_failure.delete_many(
            {'job_id': self.id})
    ...
    def remove_validation_jobs(self, validation_scenarios=None):
        ...
        if validation_scenarios != None:
            ...
            for vs_id in validation_scenarios:
                ...
                logger.debug(
                    'removing JobValidations for Job %s, using vs_id %s' % (self, vs_id))

                # attempt to retrieve JobValidation
                jvs = JobValidation.objects.filter(
                    validation_scenario=ValidationScenario.objects.get(pk=int(vs_id)), job=self)
    ...
    def _topographic_sort_jobs(job_set):
        ...
        # else, topographically sort
            ...
            lineage_edges = JobInput.objects.filter(
                job__in=job_set, input_job__in=job_set)
        ...
        if ls:
            ...
            if cancel_livy_statement:
                try:
                    ...
                    logger.debug('Livy statement cancel result: %s' %
                                 livy_response)
                except:
                    logger.debug(
                        'error canceling Livy statement: %s' % self.url)
            ...
            if kill_spark_jobs:
                try:
                    for x in range(0, 3):
                        ...
                        sj = sjs[0]
                        logger.debug('Killing Spark Job: #%s, %s' % (
                            sj['jobId'], sj.get('description', 'DESCRIPTION NOT FOUND')))
                        kill_result = SparkAppAPIClient.kill_job(
                            ls, sj['jobId'])
            ...
            logger.debug(
                'active Livy session not found, unable to cancel Livy statement or kill Spark application jobs')
    ...
    def add_input_job(self, input_job, job_spec_input_filters=None):
        ...
        if self.job_type_family() not in ['HarvestJob']:
            ...
            if JobInput.objects.filter(job=input_job, input_job=self).count() > 0:
                logger.debug(
                    'cannot add %s as Input Job, would result in circular dependency' % input_job)
                return False
            ...
            if job_spec_input_filters != None:
                ...
                input_filters['job_specific'][str(
                    input_job.id)] = job_spec_input_filters
            ...
            logger.debug('cannot add Input Job to Job type: %s' %
                         self.job_type_family())
            return False
    ...
    def remove_input_job(self, input_job):
        ...
        if self.job_type_family() not in ['HarvestJob']:
            ...
            try:
                ...
                logger.debug(
                    'input job_id not found in input_job_ids: %s' % input_job.id)
            self.update_job_details({'input_job_ids': input_job_ids})
    ...
    def remove_as_input_job(self):
        ...
        immediate_children = self.get_downstream_jobs(
            include_self=False, depth=1)
    ...
    def remove_from_published_precounts(self):
        ...
        if self.publish_set_id != '':
            ...
            subsets = PublishedRecords.get_subsets(
                includes_publish_set_id=self.publish_set_id)
            if len(subsets) > 0:
                for subset in subsets:
                    d = mc_handle.combine.misc.delete_one(
                        {'_id': 'published_field_counts_%s' % subset['name']})
                    logger.debug(d.raw_result)
    ...
    def remove_temporary_files(self):
        ...
        # else, skip
            ...
            logger.debug(
                'Job type %s, does not have remove_temporary_files() method, skipping' % cjob.job.job_type)
...
class JobInput(models.Model):
    ...
    job = models.ForeignKey(Job, on_delete=models.CASCADE)
    input_job = models.ForeignKey(
        Job, on_delete=models.CASCADE, related_name='input_job')
...
class OAIEndpoint(models.Model):
    ...
    # expecting one of setList, whiteList, blackList
    scope_type = models.CharField(
        max_length=128, null=True, default='harvestAllSets')
...
class Transformation(models.Model):
    ...
    def _transform_python(self, row):
        ...
        try:
            ...
            if type(trans_result) != list:
                raise Exception(
                    'Python transformation should return a list, but got type %s' % type(trans_result))
    ...
    def _rewrite_xsl_http_includes(self):
        ...
        if self.transformation_type == 'xslt':
            ...
            logger.debug(
                'XSLT transformation, checking for external HTTP includes')

            # rewrite flag
            rewrite = False

            # output dir
            transformations_dir = '%s/transformations' % settings.BINARY_STORAGE.rstrip(
                '/').split('file://')[-1]
            ...
            for i in includes:
                ...
                if href:
                    if href.lower().startswith('http'):
                        ...
                        r = requests.get(href)
                        filepath = '%s/%s' % (transformations_dir,
                                              href.split('/')[-1])
            ...
            if rewrite:
                ...
                self.payload = etree.tostring(
                    xsl, encoding='utf-8', xml_declaration=True).decode('utf-8')
...
class Record(mongoengine.Document):
    ...
    def get_record_stages(self, input_record_only=False, remove_duplicates=True):
        ...
        def get_downstream(record):
            ...
            downstream_job_query = JobInput.objects.filter(
                input_job=record.job)
    ...
    def get_es_doc(self, drop_combine_fields=False):
        ...
        if drop_combine_fields:
            s = s.source(exclude=['combine_id', 'db_id',
                                  'fingerprint', 'publish_set_id', 'record_id'])
            ...
            if len(mapped_dpla_fields) > 0:
                ...
                if not search_string:
                        ...
                        if local_mapped_field in mapped_dpla_fields.keys():
                            ...
                            if type(field_value) == list:
                                ...
                                for val in field_value:
                                    search_string = urllib.parse.urlencode(
                                        {target_dpla_field: '"%s"' % val})
                                    match_results = self.dpla_api_record_match(
                                        search_string=search_string)

                            # else if string, perform search
                            else:
                                search_string = urllib.parse.urlencode(
                                    {target_dpla_field: '"%s"' % field_value})
                                match_results = self.dpla_api_record_match(
                                    search_string=search_string)
                    ...
                    try:
                        ...
                        logger.debug('DPLA API call unsuccessful: code: %s, response: %s' % (
                            api_q.status_code, api_q.content))
                        self.dpla_api_doc = None
                        return self.dpla_api_doc
                    ...
                    if type(api_r) == dict:
                        if 'count' in api_r.keys():
                            ...
                            logger.debug(
                                'non-JSON response from DPLA API: %s' % api_r)
                            if not hasattr(self, "dpla_api_matches"):
                                self.dpla_api_matches = {}
    ...
    def get_record_diff(self,
        ...
        if input_record:
            ...
            logger.debug(
                'input record or XML string required, returning false')
            return False
        ...
        if output in ['all', 'combined_gen']:
            ...
            # return as HTML
            if combined_as_html:
                combined_gen = self._return_combined_diff_gen_as_html(
                    combined_gen)
        ...
        for func in pyvs_funcs:
            ...
            # if problem, report as failure with Exception string
                ...
                results_dict['failed'].append(
                    "test '%s' had exception: %s" % (func.__name__, str(e)))
...
class JobValidation(models.Model):
    ...
    validation_scenario = models.ForeignKey(
        ValidationScenario, on_delete=models.CASCADE)
    ...
    def validation_failure_count(self, force_recount=False):
        ...
        if (self.failure_count is None and self.job.finished) or force_recount:
            logger.debug(
                "calculating failure count for validation job: %s" % self)
...
class RecordValidation(mongoengine.Document):
    ...
    def validation_scenario(self):
        ...
        if self._validation_scenario is None:
            validation_scenario = ValidationScenario.objects.get(
                pk=self.validation_scenario_id)
...
class RecordIdentifierTransformationScenario(models.Model):
    ...
    transformation_type = models.CharField(
        max_length=255,
        choices=[('regex', 'Regular Expression'), ('python',
                                                   'Python Code Snippet'), ('xpath', 'XPath Expression')]
    )
    transformation_target = models.CharField(
        max_length=255,
        choices=[('record_id', 'Record Identifier'),
                 ('document', 'Record Document')]
    )
    regex_match_payload = models.CharField(
        null=True, default=None, max_length=4096, blank=True)
    regex_replace_payload = models.CharField(
        null=True, default=None, max_length=4096, blank=True)
    python_payload = models.TextField(null=True, default=None, blank=True)
    xpath_payload = models.CharField(
        null=True, default=None, max_length=4096, blank=True)
...
class DPLABulkDataDownload(models.Model):
    ...
    uploaded_timestamp = models.DateTimeField(
        null=True, default=None, auto_now_add=False)
...
class CombineBackgroundTask(models.Model):
    ...
    def calc_elapsed_as_string(self):
        ...
        if self.completed and self.finish_timestamp != None:
            ...
            seconds_elapsed = (self.finish_timestamp.replace(
                tzinfo=None) - self.start_timestamp.replace(tzinfo=None)).seconds
        ...
        # else, calc until now
            ...
            seconds_elapsed = (datetime.datetime.now() -
    ...
    def cancel(self):
        ...
        if 'job_id' in self.task_params.keys():
            ...
            logger.debug(
                'attempt to kill spark jobs related to Job: %s' % job_id)
...
class StateIO(mongoengine.Document):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.name == None:
            self.name = 'StateIO %s @ %s' % (self.stateio_type.title(
    ...
    def pre_delete(cls, sender, document, **kwargs):
        ...
        try:
            ...
            # if import
                ...
                # check for export_path and delete
                logger.debug('removing import_path: %s' %
                             document.import_manifest['import_path'])
    ...
    def export_all(self, skip_dict=None):
        ...
        org_ids = [org.id for org in Organization.objects.filter(
            for_analysis=False)]
        ...
        for conf_model in [
            ...
            conf_model_instances.extend(['%s|%s' % (
                config_prefix_hash[conf_model], conf_instance.id) for conf_instance in conf_model.objects.all()])
    ...
    def bg_task(self):
        ...
        if self.bg_task_id != None:
            # run query
            ct_task_query = CombineBackgroundTask.objects.filter(
                id=int(self.bg_task_id))
...
def user_login_handle_livy_sessions(sender, user, **kwargs):
    ...
    # else, continune with user sessions
        ...
        livy_sessions = LivySession.objects.filter(
            status__in=['starting', 'running', 'idle'])
        ...
        # if sessions present
            ...
            logger.debug(
                'multiple Livy sessions found, sending to sessions page to select one')
...
def delete_job_pre_delete(sender, instance, **kwargs):
    ...
    if instance.job_output and instance.job_output.startswith('file://'):
        ...
        try:
            ...
            logger.debug('could not remove job output directory at: %s' %
                         instance.job_output)
...
def save_transformation_to_disk(sender, instance, **kwargs):
    ...
    if instance.filepath:
        try:
            ...
            logger.debug('could not remove transformation file: %s' %
                         instance.filepath)
...
def save_validation_scenario_to_disk(sender, instance, **kwargs):
    ...
    # check that transformation directory exists
    validations_dir = '%s/validation' % settings.BINARY_STORAGE.rstrip(
        '/').split('file://')[-1]
    ...
    if instance.filepath:
        try:
            ...
            logger.debug(
                'could not remove validation scenario file: %s' % instance.filepath)
...
def background_task_pre_delete_django_tasks(sender, instance, **kwargs):
    ...
    if instance.task_output != {} and 'export_dir' in instance.task_output.keys():
        try:
            logger.debug('removing task export dir: %s' %
                         instance.task_output['export_dir'])
...
class LivyClient(object):
    ...
    def submit_job(self,
        ...
        if session_id != None:
            ...
            # submit statement
            job = self.http_request(
                'POST', 'sessions/%s/statements' % session_id, data=json.dumps(python_code))
            return job
...
class SparkAppAPIClient(object):
    ...
    def get_application_id(self, livy_session, livy_session_id):
        ...
        applications = self.http_request(
            livy_session, 'GET', '/api/v1/applications').json()

        # loop through and look for Livy session
        for app in applications:
            if app['name'] == 'livy-session-%s' % livy_session_id:
                logger.debug(
                    'found application matching Livy session id: %s' % app['id'])
                return app['id']
    ...
    def get_spark_jobs_by_jobGroup(self, livy_session, spark_app_id, jobGroup, parse_dates=False, calc_duration=True):
        ...
        jobs = self.http_request(
            livy_session, 'GET', '/api/v1/applications/%s/jobs' % spark_app_id).json()

        # loop through and filter
        filtered_jobs = [job for job in jobs if 'jobGroup' in job.keys(
        ) and job['jobGroup'] == str(jobGroup)]

        # convert to datetimes
        if parse_dates:
            for job in filtered_jobs:
                job['submissionTime'] = dateutil.parser.parse(
                    job['submissionTime'])
                if 'completionTime' in job.keys():
                    job['completionTime'] = dateutil.parser.parse(
                        job['completionTime'])
        ...
        if calc_duration:
            for job in filtered_jobs:
                ...
                # calc and append
                job['duration'] = (ct.replace(tzinfo=None) -
                                   st.replace(tzinfo=None)).seconds
    ...
    def kill_job(self, livy_session, job_id):
        ...
        kill_request = self.http_request(
            livy_session, 'GET', '/jobs/job/kill/', params={'id': job_id})
    ...
    def _calc_field_metrics(self,
        ...
        if sr_dict['aggregations']['%s_doc_instances' % field_name]['doc_count'] > 0:
            ...
            if field_dict['val_instances'] > 0:
                field_dict['distinct_ratio'] = round(
                    (field_dict['distinct'] / field_dict['val_instances']), 4)
            else:
    ...
    def count_indexed_fields(self,
        ...
        if self.es_index != [] and es_handle.indices.exists(index=self.es_index) and es_handle.search(index=self.es_index)['hits']['total'] > 0:
            ...
            for field_name in field_names:
                ...
                # for field_name in field_names:
                s.aggs.bucket('%s_doc_instances' % field_name, A(
                    'filter', Q('exists', field=field_name)))
                s.aggs.bucket('%s_val_instances' % field_name, A(
                    'value_count', field='%s.keyword' % field_name))
            ...
            if job_record_count:
                indexed_percentage = round(
                    (float(return_dict['total_docs']) / float(job_record_count)), 4)
...
class PublishedRecords(object):
    ...
    def __init__(self, subset=None):
        ...
        if self.subset != None:
            ...
            self.ps_doc = mc_handle.combine.misc.find_one(
                {'type': 'published_subset', 'name': self.subset})

            if self.ps_doc == None:
                logger.warning(
                    'published subset could not be found for name: %s' % self.subset)
                ...
                # filter jobs to sets
                self.published_jobs = self.published_jobs.filter(
                    publish_set_id__in=publish_set_ids)
                ...
                if self.ps_doc.get('include_non_set_records', False):
                    non_set_published_jobs = Job.objects.filter(
                        published=True, publish_set_id='')
                ...
                if len(hierarchy) > 0:
                    ...
                    record_group_ids = [
                        int(_.split('|')[1]) for _ in hierarchy if _.startswith('record_group')]
                    job_ids = [int(_.split('|')[1])
    ...
    def get_record(self, record_id):
        ...
        if record_query.count() == 1:
            ...
            logger.debug(
                'multiple records found for id %s - this is not allowed for published records' % id)
            return False
    ...
    def count_indexed_fields(self, force_recount=False):
        ...
        published_field_counts = mc_handle.combine.misc.find_one(
            self.mongo_count_id)
        ...
        # else, calculate, store, and return
            ...
            logger.debug(
                'calculating published field counts, saving, and returning')
    ...
    def get_publish_set_ids():
        ...
        publish_set_ids = Job.objects.exclude(
            publish_set_id=None).values('publish_set_id').distinct()
    ...
    def remove_subset_precounts(self):
        ...
        mc_handle.combine.misc.delete_one(
            {'_id': 'published_field_counts_%s' % self.subset})
...
class CombineJob(object):
    ...
    def init_combine_job(
        ...
        job_details = CombineJob._parse_shared_job_params(
            job_details, job_params, kwargs)

        # capture and mix in job type specific params
        job_details = job_type_class.parse_job_type_params(
            job_details, job_params, kwargs)
    ...
    def _parse_shared_job_params(job_details, job_params, kwargs):
        ...
        job_details['input_filters'] = CombineJob._parse_input_filters(
            job_params)

        # get requested validation scenarios and convert to int
        job_details['validation_scenarios'] = [
            int(vs_id) for vs_id in job_params.getlist('validation_scenario', [])]
        ...
        if job_details['dbdm']['dbdd'] != None:
            ...
            # get dbdd instance
            dbdd = DPLABulkDataDownload.objects.get(
                pk=job_details['dbdm']['dbdd'])
            # set s3_key
    ...
    def _parse_input_filters(job_params):
        ...
        # validity valve
        input_filters['input_validity_valve'] = job_params.get(
        ...
        # duplicates valve
        filter_dupe_record_ids = job_params.get(
        ...
        for job_spec_json in job_specs:
            ...
            job_spec_form_dict = {f['name'].lstrip(
                'job_spec_'): f['value'] for f in json.loads(job_spec_json)}

            # prepare job_spec
            job_spec_dict = {}

            # validity valve
            job_spec_dict['input_validity_valve'] = job_spec_form_dict.get(
                'input_validity_valve', 'all')

            # numerical valve
            input_numerical_valve = job_spec_form_dict.get(
                'input_numerical_valve', None)
            if input_numerical_valve in ('', None):
                job_spec_dict['input_numerical_valve'] = None
            else:
                job_spec_dict['input_numerical_valve'] = int(
                    input_numerical_valve)

            # es query valve
            input_es_query_valve = job_spec_form_dict.get(
                'input_es_query_valve', None)
            if input_es_query_valve in ('', None):
                input_es_query_valve = None
            ...
            # duplicates valve
            filter_dupe_record_ids = job_spec_form_dict.get(
                'filter_dupe_record_ids', 'true')
            if filter_dupe_record_ids == 'true':
    ...
    def write_input_job_links(self, job_details):
        ...
        # get input_jobs
        input_jobs = [Job.objects.get(pk=int(job_id))
                      for job_id in job_details['input_job_ids']]
    ...
    def get_record(self, id, record_field='record_id'):
        ...
        record_query = Record.objects.filter(
            job=self.job).filter(**{record_field: id})
    ...
    def get_job_output_filename_hash(self):
        ...
        try:
            avros = [f for f in os.listdir(
                job_output_dir) if f.endswith('.avro')]

            if len(avros) > 0:
                job_output_filename_hash = re.match(
                    r'part-[0-9]+-(.+?)\.avro', avros[0]).group(1)
                logger.debug('job output filename hash: %s' %
                             job_output_filename_hash)
            ...
            logger.debug(
                'could not load job output to determine filename hash')
            return False
    ...
    def rerun(self, rerun_downstream=True, set_gui_status=True):
        ...
        for re_job in rerun_jobs:
            ...
            if re_cjob.job.spark_code != None:
                ...
                logger.debug(
                    'Spark code not set for Job, attempt to re-prepare...')
                re_cjob.job.spark_code = re_cjob.prepare_job(
                    return_job_code=True)
                re_cjob.job.save()
                re_cjob.submit_job_to_livy(eval(re_cjob.job.spark_code))
    ...
    def clone(self, rerun=True, clone_downstream=True, skip_clones=[]):
        ...
        for job in to_clone:
                ...
                if ji.input_job in clones.keys():
                    ...
                    for job_id in update_dict['input_filters']['job_specific'].keys():
                        ...
                        if int(job_id) in clones_ids.keys():
                            job_spec_dict = update_dict['input_filters']['job_specific'].pop(
                                job_id)
                            update_dict['input_filters']['job_specific'][str(
                                clones_ids[ji.input_job.id])] = job_spec_dict
            ...
            for jv in job.jobvalidation_set.all():
                logger.debug('cloning validation link: %s' %
                             jv.validation_scenario.name)
                jv.pk = None
                jv.job = clone.job
                jv.save()
...
class HarvestJob(CombineJob):
    ...
    def __init__(self,
        ...
        if not job_id:
            ...
            self.job = Job(
                record_group=record_group,
                # selects this level of class inheritance hierarchy
...
class HarvestOAIJob(HarvestJob):
    ...
    def parse_job_type_params(job_details, job_params, kwargs):
        ...
        oai_params = OAIEndpoint.objects.get(
            pk=int(job_params.get('oai_endpoint_id'))).__dict__.copy()
        ...
        include_oai_record_header = job_params.get(
            'include_oai_record_header', False)
    ...
    def parse_job_type_params(job_details, job_params, kwargs):
        ...
        # When a payload is uploaded, create payload_dir and set
            ...
            if kwargs['hash_payload_filename']:
                job_details['payload_filename'] = hashlib.md5(
                    payload_file.name.encode('utf-8')).hexdigest()
            else:
            ...
            if job_details['content_type'] in ['application/zip', 'application/x-zip-compressed']:
                ...
                zip_filepath = os.path.join(
                    job_details['payload_dir'], job_details['payload_filename'])
        ...
        job_details['xpath_document_root'] = job_params.get(
            'xpath_document_root', None)
        job_details['document_element_root'] = job_params.get(
            'document_element_root', None)
        job_details['additional_namespace_decs'] = job_params.get(
            'additional_namespace_decs', None).replace("'", '"')
        job_details['xpath_record_id'] = job_params.get(
    ...
    def remove_temporary_files(self):
        ...
        if payload_dir and os.path.exists(payload_dir):
            ...
            if len(payload_deps) == 0:
                ...
                logger.debug(
                    'NOT removing payload_dir, shared by other Jobs: %s' % payload_deps)
...
class HarvestTabularDataJob(HarvestJob):
    ...
    def parse_job_type_params(job_details, job_params, kwargs):
        ...
        # use upload
            ...
            # filepath
            job_details['payload_filepath'] = os.path.join(
                job_details['payload_dir'], job_details['payload_filename'])
        ...
        # get fm_config_json
        job_details['fm_harvest_config_json'] = job_params.get(
            'fm_harvest_config_json')
...
class TransformJob(CombineJob):
    ...
    def parse_job_type_params(job_details, job_params, kwargs):
        ...
        # retrieve input jobs
        job_details['input_job_ids'] = [
            int(job_id) for job_id in job_params.getlist('input_job_id')]
...
class AnalysisJob(CombineJob):
    ...
    def get_analysis_hierarchy():
        ...
        # if one found, use
            ...
            raise Exception(
                'multiple Organizations found for explicit purpose of aggregating Analysis jobs')

        # check of Analysis jobs aggregating Record Group exists
        analysis_record_group_search = RecordGroup.objects.filter(
            name=record_group_name)
        if analysis_record_group_search.count() == 0:
            logger.debug('creating RecordGroup with name %s' %
                         record_group_name)
        ...
        # if one found, use
            ...
            raise Exception(
                'multiple Record Groups found for explicit purpose of aggregating Analysis jobs')
...
class DTElasticFieldSearch(View):
    ...
    def filter(self):
        ...
        if self.DTinput['search[value]'] != '':
            ...
            self.query = self.query.query(
                'match', _all=self.DTinput['search[value]'])
    ...
    def sort(self):
        ...
        if self.search_type == 'fields_per_doc':
            ...
            if sort_col < len(self.fields):
                ...
                logger.debug("sortable field, sorting by %s, %s" %
                             (sort_field_string, sort_dir))
            else:
        ...
        if self.search_type == 'values_per_field':
            ...
            if sort_col < len(self.query_results.columns):
                ...
                if sort_dir == 'desc':
                    asc = False
                self.query_results = self.query_results.sort_values(
                    self.query_results.columns[sort_col], ascending=asc)
    ...
    def values_per_field(self, terms_limit=10000):
        ...
        if self.request:
            ...
            # expects only one for this search type, take first
            self.field = self.fields[0]
        ...
        # add agg bucket for field values
        self.query.aggs.bucket(self.field, A(
            'terms', field='%s.keyword' % self.field, size=terms_limit))
        ...
        self.query_results = pd.DataFrame(
            [val.to_dict() for val in sr.aggs[self.field]['buckets']])
...
class DTElasticGenericSearch(View):
    ...
    def search(self):
        ...
        for hit in self.query_results.hits:
            ...
            try:
                ...
                row_data = self._prepare_record_hierarchy_links(
                    record, row_data)
                ...
                logger.debug("error retrieving DB record based on id %s, from index %s: %s" % (
                    hit.db_id, hit.meta.index, str(e)))
    ...
    def _prepare_record_hierarchy_links(self, record, row_data):
        ...
        to_append = [
            '<a href="%s">%s</a>' % (urls['organization']
                                     ['path'], urls['organization']['name']),
            '<a href="%s">%s</a>' % (urls['record_group']
                                     ['path'], urls['record_group']['name']),
            '<a href="%s"><span class="%s">%s</span></a>' % (
                urls['job']['path'], record.job.job_type_family(), urls['job']['name']),
            urls['record']['path'],
        ]
...
class RITSClient(object):
    ...
    def __init__(self, query_dict):
        ...
        if self.qd.get('record_id_transform_type', None) == 'regex':
            ...
            logger.debug('parsing as %s type transformation' %
                         self.transform_type)
    ...
    def test_user_input(self):
        ...
        if self.transform_type == 'regex':
            trans_result = re.sub(
                self.regex_match, self.regex_replace, self.test_input)
        ...
        if self.transform_type == 'python':
            ...
            if self.target == 'record_id':
                sr = PythonUDFRecord(
                    None, non_row_input=True, record_id=self.test_input)
            if self.target == 'document':
                sr = PythonUDFRecord(
                    None, non_row_input=True, document=self.test_input)
        ...
        if self.transform_type == 'xpath':
            ...
            if self.target == 'document':
                ...
                # attempt xpath
                xpath_results = sr.xml.xpath(
                    self.xpath_payload, namespaces=sr.nsmap)
                n = xpath_results[0]
                trans_result = n.text
...
class DPLABulkDataClient(object):
    ...
    def __init__(self):
        ...
        self.bulk_dir = '%s/bulk' % settings.BINARY_STORAGE.rstrip(
            '/').split('file://')[-1]
...
class GlobalMessageClient(object):
    ...
    def delete_gm(self, gm_id):
        ...
        if 'gms' not in self.session:
            ...
            self.session['gms'][:] = [
                gm for gm in self.session['gms'] if gm.get('id') != gm_id]
            self.session.save()
...
class StateIOClient(object):
    ...
    def _confirm_working_dirs(self):
        ...
        if not os.path.isdir(settings.STATEIO_EXPORT_DIR):
            logger.debug('creating StateIO working directory: %s' %
    ...
    def _get_unique_roots_job_set(self):
        ...
        for org in self.export_roots['orgs']:
            self.export_roots['record_groups'].extend(
                org.recordgroup_set.all())
    ...
    def _collect_related_jobs(self, include_upstream=True):
            ...
            if include_upstream:
                ...
                for job in self.export_dict['jobs']:
                    ...
                    for lineage_job_dict in lineage['nodes']:
                        upstream_jobs.append(Job.objects.get(
                            pk=int(lineage_job_dict['id'])))
        ...
        self.export_dict['jobs'] = Job._topographic_sort_jobs(
            self.export_dict['jobs'])
        self.export_manifest['jobs'] = [
            job.id for job in self.export_dict['jobs']]
    ...
    def _collect_related_components(self):
        ...
        # get all related Job Inputs
        job_validations = JobValidation.objects.filter(
            job__in=self.export_dict['jobs'])
            ...
            if 'transformation' in job.job_details_dict.keys():
                ...
                try:
                    for trans in job.job_details_dict['transformation']['scenarios']:
                        self.export_dict['transformations'].add(
                            Transformation.objects.get(pk=int(trans['id'])))
                except Exception as e:
                    logger.warning(
                        'Could not export Transformations for job %s: %s' % (job, str(e)))
        ...
        for job in self.export_dict['jobs']:
            ...
            if job.job_type == 'HarvestOAIJob':
                ...
                try:
                    ...
                    self.export_dict['oai_endpoints'].add(OAIEndpoint.objects.get(
                        pk=job.job_details_dict['oai_params']['id']))
                except Exception as e:
                    logger.warning(
                        'Could not export OAIEndpoint for job %s: %s' % (job, str(e)))
        ...
        for job in self.export_dict['jobs']:
            ...
            if 'rits' in job.job_details_dict.keys() and job.job_details_dict['rits'] != None:
                try:
                    self.export_dict['rits'].add(RecordIdentifierTransformationScenario.objects.get(
                        pk=(job.job_details_dict['rits'])))
                except Exception as e:
                    logger.warning(
                        'Could not export Record Identifier Transformation Scenario for job %s: %s' % (job, str(e)))
        ...
        for job in self.export_dict['jobs']:
            ...
            if 'dbdm' in job.job_details_dict.keys() and job.job_details_dict['dbdm']['dbdd'] != None:
                ...
                logger.debug('attempting to export dbdd_id %s for %s' %
                             (job.job_details_dict['dbdm']['dbdd'], job))

                try:
                    # get dbdd
                    dbdd = DPLABulkDataDownload.objects.get(
                        pk=(job.job_details_dict['dbdm']['dbdd']))
                    ...
                    cmd = [
                        "elasticdump",
                        "--input=http://%s:9200/%s" % (
                            settings.ES_HOST, dbdd.es_index),
                        "--output=%(dbdd_export_path)s/dbdd%(dbdd_id)s.json" % {
                            'dbdd_export_path': dbdd_export_path, 'dbdd_id': dbdd.id},
                        "--type=data",
                        "--ignore-errors",
                        "--noRefresh"
                    ]
                    ...
                    logger.debug(
                        'could not export DBDD for job %s: %s' % (job, str(e)))
        ...
        for job in self.export_dict['jobs']:
            ...
            cmd = [
                ...
                "--output=%(es_export_path)s/j%(job_id)s_mapped_fields.json" % {
                    'es_export_path': es_export_path, 'job_id': job.id},
                "--type=data",
                "--sourceOnly",
                "--ignore-errors",
                "--noRefresh"
    ...
    def _sort_discrete_config_scenarios(self, config_scenarios):
        ...
        for config_scenario in config_scenarios:
            ...
            logger.debug(
                'adding to export_dict for serialization: %s' % config_scenario)
            ...
            if type(config_scenario) == str and '|' in config_scenario:
                ...
                config_scenario = id_prefix_hash[model_type].objects.get(
                    pk=int(model_id))

            # slot to export dict through hash
            self.export_dict[sorting_hash[type(config_scenario)]].add(
                config_scenario)
    ...
    def package_export(self):
        ...
        if self.compress:
            ...
            export_filename = '%s/%s' % (settings.STATEIO_EXPORT_DIR,
            ...
            logger.debug('archive %s created in %ss' %
                         (new_export_path, (time.time() - compress_stime)))
    ...
    def _collect_published_subsets(self):
        ...
        non_publish_set_jobs = [
            job for job in self.export_dict['jobs'] if job.publish_set_id == '']
        non_publish_set_subsets = [subset for subset in PublishedRecords.get_subsets(
        ) if subset['include_non_set_records']]
        if len(non_publish_set_jobs) > 0 and len(non_publish_set_subsets) > 0:
            self.export_manifest['published_subsets'].extend(
                non_publish_set_subsets)
        ...
        for job in self.export_dict['jobs']:
            ...
            if job.published and job.publish_set_id != '':
                self.export_manifest['published_subsets'].extend(
                    PublishedRecords.get_subsets(includes_publish_set_id=job.publish_set_id))
    ...
    def _prepare_files(self):
        ...
        self.import_path = '%s/%s' % (settings.STATEIO_IMPORT_DIR,
                                      self.import_id)
        ...
        if self.export_path.startswith('http'):
            logger.debug(
                'exported state determined to be URL, downloading and processing')
            raise Exception(
                'not yet handling remote URL locations for importing states')

        # handle archives
        elif os.path.isfile(self.export_path) and self.export_path.endswith(('.zip', '.tar', '.tar.gz')):
            logger.debug(
                'exported state determined to be archive, decompressing')
            shutil.unpack_archive(self.export_path, self.import_path)

        # handle dir
        elif os.path.isdir(self.export_path):
            logger.debug(
                'exported state is directory, copying to import directory')
            os.system('cp -r %s %s' % (self.export_path, self.import_path))

        # else, raise Exception
        else:
            raise Exception(
                'cannot handle export_path %s, aborting' % self.export_path)
    ...
    def _import_config_instances(self):
        ...
        for ts in self._get_django_model_type(Transformation):
            ...
            ts_match = Transformation.objects.filter(
                name=ts.object.name, payload=ts.object.payload).order_by('id')

            # matching scenario found
            if ts_match.count() > 0:
                logger.debug(
                    'found identical Transformation, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['transformations'][ts.object.id] = ts_match.first(
                ).id
        ...
        for vs in self._get_django_model_type(ValidationScenario):
            ...
            # check for identical name and payload
            vs_match = ValidationScenario.objects.filter(
                name=vs.object.name, payload=vs.object.payload).order_by('id')

            # matching scenario found
            if vs_match.count() > 0:
                logger.debug(
                    'found identical ValidationScenario, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['validations'][vs.object.id] = vs_match.first(
                ).id
        ...
        for oai in self._get_django_model_type(OAIEndpoint):
            ...
            # check for identical name and payload
            oai_match = OAIEndpoint.objects.filter(
                name=oai.object.name, endpoint=oai.object.endpoint).order_by('id')

            # matching scenario found
            if oai_match.count() > 0:
                logger.debug(
                    'found identical OAIEndpoint, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['oai_endpoints'][oai.object.id] = oai_match.first(
                ).id
        ...
        for scenario in self._get_django_model_type(FieldMapper):
            ...
            if scenario_match.count() > 0:
                logger.debug(
                    'found identical FieldMapper, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['field_mapper_configs'][scenario.object.id] = scenario_match.first(
                ).id
        ...
        for scenario in self._get_django_model_type(RecordIdentifierTransformationScenario):
            ...
            if scenario_match.count() > 0:
                logger.debug(
                    'found identical RITS, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['rits'][scenario.object.id] = scenario_match.first(
                ).id
        ...
        for scenario in self._get_django_model_type(DPLABulkDataDownload):
            ...
            if scenario_match.count() > 0:
                logger.debug(
                    'found identical DPLA Bulk Data Download, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['dbdd'][scenario.object.id] = scenario_match.first(
                ).id
            ...
            # not found, creating
                ...
                # get dbdd
                dbdd = DPLABulkDataDownload.objects.get(
                    pk=(scenario.object.id))

                # import DBDD index to ElaticSearch
                dbdd_export_path = '%s/dbdd/dbdd%s.json' % (
                    self.import_path, orig_id)

                # build command list
                cmd = [
                    "elasticdump",
                    "--input=%(dbdd_export_path)s" % {
                        'dbdd_export_path': dbdd_export_path},
    ...
    def _import_hierarchy(self):
        ...
        for org in self._get_django_model_type(Organization):
            ...
            # check Org name
            org_match = Organization.objects.filter(
                name=org.object.name).order_by('id')

            # matching Organization found
            if org_match.count() > 0:
                logger.debug(
                    'found same Organization name, skipping creation, adding to hash')
                self.import_manifest['pk_hash']['orgs'][org.object.id] = org_match.first(
                ).id
            ...
            for rg in self._get_django_model_type(RecordGroup):
                ...
                if rg.object.organization_id == org_orig_id:
                    ...
                    if rg_match.count() > 0:
                        logger.debug(
                            'found Organization/Record Group name combination, skipping creation, adding to hash')
                        self.import_manifest['pk_hash']['record_groups'][rg.object.id] = rg_match.first(
                        ).id
    ...
    def _import_job_related_instances(self):
        ...
        for jv in self._get_django_model_type(JobValidation):
            ...
            jv.object.validation_scenario_id = self.import_manifest[
                'pk_hash']['validations'][vs_orig_id]
    ...
    def _import_db_records(self):
        ...
        submit = LivyClient().submit_job(
            LivySession.get_active_session().session_id, {'code': self.spark_code})
        self.spark_results = polling.poll(lambda: LivyClient().job_status(
            submit.headers['Location']).json()['state'] == 'available', step=5, poll_forever=True)
    ...
    def _import_published_subsets(self):
        ...
        for subset in self.export_manifest['published_subsets']:
            ...
            subset_q = mc_handle.combine.misc.find_one(
                {'type': 'published_subset', 'name': subset['name']})
            logger.debug(subset_q)

            # does not exist, create
            if subset_q == None:
                logger.debug(
                    'Published Subset %s not found, creating' % subset['name'])
            ...
            # if found, check if identical
                ...
                if subset == subset_q:
                    logger.debug(
                        'found identical Published Subset %s, skipping import' % subset['name'])

                # if different, rename and import
                else:
                    logger.debug(
                        'found subset with same name, but different properties, creating new')
    ...
    def _finalize_import_manifest(self):
        ...
        for import_type in self.import_manifest['imports'].keys():
            ...
            inv_pk_hash = {
                v: k for k, v in self.import_manifest['pk_hash'][import_type].items()}
        ...
        self.import_manifest['pk_hash_json'] = json.dumps(
            self.import_manifest['pk_hash'])
    ...
    def _update_job_details(self, job):
        ...
        if 'validation_scenarios' in job.job_details_dict.keys():
            try:
                ...
                validation_scenarios_updated = [pk_hash['validations'].get(
                    vs_id, vs_id) for vs_id in validation_scenarios]
                update_dict['validation_scenarios'] = validation_scenarios_updated
            except:
                logger.debug(
                    'error with updating job_details: validation_scenarios')

        # update input_job_ids
        if 'input_job_ids' in job.job_details_dict.keys():
            try:
                input_job_ids = job.job_details_dict['input_job_ids']
                input_job_ids_updated = [pk_hash['jobs'].get(
                    job_id, job_id) for job_id in input_job_ids]
        ...
        if 'input_filters' in job.job_details_dict.keys():
            try:
                ...
                job_specific_updated = {str(pk_hash['jobs'].get(int(k), int(
                    k))): v for k, v in input_filters['job_specific'].items()}
        ...
        if job.job_type == 'HarvestOAIJob':
            try:
                ...
                oai_params['id'] = pk_hash['oai_endpoints'].get(
                    oai_params['id'], oai_params['id'])
        ...
        if job.job_type == 'TransformJob':
            try:
                ...
                transformation['id'] = pk_hash['transformations'].get(
