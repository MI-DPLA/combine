...
class JobTestCase(TestCase):
    ...
    def test_job_harvest_static_xml_form(self):
        response = self.client.get(f'/combine/organization/{self.org.id}/record_group/{self.record_group.id}/job/harvest/static/xml/new')
        self.assertIn(b"Harvest records from static XML files", response.content)
